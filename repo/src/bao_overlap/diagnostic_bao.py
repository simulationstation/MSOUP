...
from .bao_template import bao_template_damped, bao_wiggle_components


class FitFailure(RuntimeError):
    """Raised when diagnostic fits collapse to alpha boundaries."""
...
@dataclass
class WiggleOnlyFitResult:
    alpha: float
    sigma_nl: float | None
    chi2: float
    dof: int
    alpha_hit_bounds: bool
    sigma_nl_hit_bounds: bool
    meta: Dict[str, Any]
...
def _poly2_design(s: np.ndarray) -> np.ndarray:
...
def fit_wiggle_only_monopole(
    ...
    alpha_bounds: Tuple[float, float] = (0.6, 1.4),
    alpha_scan_points: int = 41,
    sigma_nl_bounds: Tuple[float, float] | None = None,
    optimizer: str = "L-BFGS-B",
) -> WiggleOnlyFitResult:
    mask = (s >= fit_range[0]) & (s <= fit_range[1])
    s_fit = s[mask]
    ...
    broadband = _poly2_design(s_fit)

    def chi2_for_alpha_sigma(alpha: float, sigma_nl: float | None) -> Tuple[float, np.ndarray]:
        ...
        design = np.column_stack([xi_wiggle, broadband])
        lhs = design.T @ inv_cov @ design
        rhs = design.T @ inv_cov @ xi_fit
        coeffs = np.linalg.solve(lhs, rhs)
        model = design @ coeffs
    ...
    sigma_fixed = template_params["sigma_nl"]
    sigma_hit_bounds = False
    for idx, alpha_val in enumerate(alpha_grid):
        if sigma_nl_bounds is None:
            sigma_best = sigma_fixed
            chi2_best, coeffs = chi2_for_alpha_sigma(alpha_val, sigma_best)
        else:
            def objective(sigma_arr: np.ndarray) -> float:
                sigma_val = float(sigma_arr[0])
            ...
            sigma_tol = 1.0e-3
            sigma_hit_bounds = sigma_hit_bounds or (
                abs(sigma_best - sigma_nl_bounds[0]) <= sigma_tol
                or abs(sigma_best - sigma_nl_bounds[1]) <= sigma_tol
            )
    ...
    best_idx = int(np.argmin(chi2_grid))
    alpha_hat = float(alpha_grid[best_idx])
    sigma_nl_hat = float(sigma_grid[best_idx]) if sigma_nl_bounds is not None else None
    coeffs = coeff_grid[best_idx]
    chi2_min = float(chi2_grid[best_idx])

    n_params = 1 + broadband.shape[1] + (1 if sigma_nl_bounds is not None else 0)
    dof = max(len(s_fit) - n_params, 1)

    alpha_tol = 1.0e-3
    alpha_hit_bounds = (
        abs(alpha_hat - alpha_bounds[0]) <= alpha_tol
        or abs(alpha_hat - alpha_bounds[1]) <= alpha_tol
    )

    xi_lin, xi_nowiggle, _ = bao_wiggle_components(
        alpha_hat * s_fit,
        r_d=template_params["r_d"],
        omega_m=template_params["omega_m"],
        omega_b=template_params["omega_b"],
    ...
    meta = {
        ...
        "chi2_grid": chi2_grid.tolist(),
        "sigma_nl_grid": sigma_grid.tolist(),
        "broadband_terms": ["1", "s", "s^2"],
        "broadband_coeffs": coeffs[1:].tolist(),
        "bias_coeff": float(coeffs[0]),
        "optimizer": optimizer,
        "multipoles": ["xi0"],
        "xi_lin": xi_lin.tolist(),
        "xi_nowiggle": xi_nowiggle.tolist(),
    }

    return WiggleOnlyFitResult(
        alpha=alpha_hat,
        sigma_nl=sigma_nl_hat,
        chi2=chi2_min,
        dof=dof,
        alpha_hit_bounds=alpha_hit_bounds,
        sigma_nl_hit_bounds=sigma_hit_bounds,
        meta=meta,
    )
